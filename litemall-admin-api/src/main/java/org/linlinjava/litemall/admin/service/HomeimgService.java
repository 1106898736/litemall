...
public class HomeimgService {
    private final Log logger = LogFactory.getLog(HomeimgService.class);
    ...
    @Autowired
    private LitemallHomeimgService introduceService;

    @Autowired
    private LitemallGoodsService goodsService1;
    @Autowired
    ...
    public Object list() {
        List<LitemallHomeimg> goodsList = introduceService.querySelective();
     ...
     */
    @Transactional
    public int update(LitemallHomeimg introduce) {
        introduce.setUpdateTime(LocalDateTime.now());
        logger.info(introduce);
        return homeimgMapper.updateByPrimaryKeySelective(introduce);
    }

    public void add(LitemallIntroduce ad) {
        ad.setAddTime(LocalDateTime.now());
        ad.setUpdateTime(LocalDateTime.now());
        introduceMapper.insertSelective(ad);
    }

    @Transactional
    public Object delete(LitemallIntroduce goods) {
        ...
        introduceService.deleteById(gid);
    ...
    public Object create(GoodsAllinone goodsAllinone) {
        ...
        String name = goods.getName();
        if (introduceService.checkExistByName(name)) {
        ...
        goodsService1.add(goods);

        //将生成的分享图片地址写入数据库
        String url = qCodeService.createGoodShareImage(goods.getId().toString(), goods.getPicUrl(), goods.getName());
        if (!StringUtils.isEmpty(url)) {
            goods.setShareUrl(url);
            if (goodsService1.updateById(goods) == 0) {
    ...
    public Object detail(Integer id) {
        LitemallIntroduce goods = introduceService.findById(id);
        ...
        Integer categoryId = goods.getId();
